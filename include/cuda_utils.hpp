...
static inline bool __cuda_check_error(cudaError_t code, const char* file, int line) {
    if (code != cudaSuccess) {
        std::cerr << "CUDA Error:\n";
        std::cerr << "    File:       " << file << "\n";
        std::cerr << "    Line:       " << line << "\n";
...
public:
    ...
    int64_t hostSize() const {
        return _host_bytes / _dtype_bytes;
    }

    int64_t deviceSize() const {
        return _device_bytes / _dtype_bytes;
    }
}; // class HostDeviceMem
...
public:
    ...
    bool construct(const void* pdata, size_t size) {
        ...
        runtime = std::shared_ptr<nvinfer1::IRuntime>(nvinfer1::createInferRuntime(gLogger), destroy_ptr<nvinfer1::IRuntime>);
        if (runtime == nullptr) return false;

        engine = std::shared_ptr<nvinfer1::ICudaEngine>(runtime->deserializeCudaEngine(pdata, size, nullptr), destroy_ptr<nvinfer1::ICudaEngine>);
        if (engine == nullptr) return false;

        context = std::shared_ptr<nvinfer1::IExecutionContext>(engine->createExecutionContext(), destroy_ptr<nvinfer1::IExecutionContext>);

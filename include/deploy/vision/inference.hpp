...
public:
    // Use static_assert to ensure that T is either DetResult, OBBResult, SegResult, PoseResult, or ClsResult.
    static_assert(
        std::is_same<T, DetResult>::value ||
            std::is_same<T, OBBResult>::value ||
            std::is_same<T, SegResult>::value ||
            std::is_same<T, PoseResult>::value ||
            std::is_same<T, ClsResult>::value,
        "T must be either DetResult, OBBResult, SegResult, PoseResult, or ClsResult.");
...
template class BaseTemplate<ClsResult>;
template class DeployTemplate<ClsResult>;
template class DeployCGTemplate<ClsResult>;
...
typedef BaseTemplate<ClsResult>      BaseCls;
typedef DeployTemplate<ClsResult>    DeployCls;
typedef DeployCGTemplate<ClsResult>  DeployCGCls;

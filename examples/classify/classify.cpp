...
// 在图像上可视化推理结果
void visualize(cv::Mat& image, trtyolo::ClassifyRes& result, const std::vector<std::string>& labels) {
...
// 处理单张图像
void process_single_image(const std::string& image_path, const std::string& output_path, trtyolo::ClassifyModel& model, const std::vector<std::string>& labels) {
...
// 处理一批图像
void process_batch_images(const std::vector<std::string>& image_paths, const std::string& output_path, trtyolo::ClassifyModel& model, const std::vector<std::string>& labels) {
...
int main(int argc, char** argv) {
    try {
        ...
        auto model = std::make_unique<trtyolo::ClassifyModel>(engine_path, option);

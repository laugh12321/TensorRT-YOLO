...
#include "efficientIdxNMSInference.cuh"
...
template <typename T>
__device__ float IOU(EfficientIdxNMSParameters param, BoxCorner<T> box1, BoxCorner<T> box2)
{
...
__device__ BoxCorner<T> DecodeBoxes(EfficientIdxNMSParameters param, int boxIdx, int anchorIdx,
...
template <typename T, typename Tb>
__device__ void MapNMSData(EfficientIdxNMSParameters param, int idx, int imageIdx, const Tb* __restrict__ boxesInput,
...
template <typename T>
__device__ void WriteNMSResult(EfficientIdxNMSParameters param, int* __restrict__ numDetectionsOutput,
...
template <typename T, typename Tb>
__global__ void EfficientNMS(EfficientIdxNMSParameters param, const int* topNumData, int* outputIndexData,
...
template <typename T>
cudaError_t EfficientNMSLauncher(EfficientIdxNMSParameters& param, int* topNumData, int* outputIndexData,
...
__global__ void EfficientNMSFilterSegments(EfficientIdxNMSParameters param, const int* __restrict__ topNumData,
...
template <typename T>
__global__ void EfficientNMSFilter(EfficientIdxNMSParameters param, const T* __restrict__ scoresInput,
...
template <typename T>
__global__ void EfficientNMSDenseIndex(EfficientIdxNMSParameters param, int* __restrict__ topNumData,
...
template <typename T>
cudaError_t EfficientNMSFilterLauncher(EfficientIdxNMSParameters& param, const T* scoresInput, int* topNumData,
...
size_t EfficientIdxNMSWorkspaceSize(int batchSize, int numScoreElements, int numClasses, DataType datatype)
{
    size_t total = 0;
...
template <typename T>
pluginStatus_t EfficientNMSDispatch(EfficientIdxNMSParameters param, const void* boxesInput, const void* scoresInput,
...
pluginStatus_t EfficientIdxNMSInference(EfficientIdxNMSParameters param, const void* boxesInput, const void* scoresInput,
    const void* anchorsInput, void* numDetectionsOutput, void* nmsBoxesOutput, void* nmsScoresOutput,
    void* nmsClassesOutput, void* nmsIndicesOutput, void* workspace, cudaStream_t stream)
{

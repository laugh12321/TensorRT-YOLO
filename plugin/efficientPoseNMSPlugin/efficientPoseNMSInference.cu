...
template <typename T>
__device__ void WritePoseNMSResult(EfficientPoseNMSParameters param, int* __restrict__ numDetectionsOutput,
    T* __restrict__ nmsScoresOutput, int* __restrict__ nmsClassesOutput,
    BoxCorner<T>* __restrict__ nmsBoxesOutput, KeyPoints<T>* __restrict__ nmsKeypointsOutput,
    T threadScore, int threadClass, BoxCorner<T> threadBox,
    int boxIdxMap, const KeyPoints<T>* __restrict__ keypointsInput, 
    int imageIdx, unsigned int resultsCounter)
{
    ...
    for (int i=0; i < 17; ++i)
    {
        nmsKeypointsOutput[outputIdx].kpts[i] = keypointsInput[boxIdxMap].kpts[i];
    }
...
__global__ void EfficientPoseNMS(EfficientPoseNMSParameters param, const int* topNumData, int* outputIndexData,
    ...
    const int* __restrict__ topClassData, const int* __restrict__ topAnchorsData, 
    const Tb* __restrict__ boxesInput, const KeyPoints<T>* __restrict__ keypointsInput, 
    const Tb* __restrict__ anchorsInput, int* __restrict__ numDetectionsOutput, T* __restrict__ nmsScoresOutput,
    int* __restrict__ nmsClassesOutput, KeyPoints<T>* __restrict__ nmsKpsOutput, BoxCorner<T>* __restrict__ nmsBoxesOutput)
{
    ...
    for (int tile = 0; tile < numTiles; tile++)
    {
        ...
        MapNMSData<T, Tb>(param, boxIdx[tile], imageIdx, boxesInput, anchorsInput, topClassData, topAnchorsData,
            topNumData, sortedScoresData, sortedIndexData, threadScore[tile], threadClass[tile], threadBox[tile], boxIdxMap[tile]);
    }
            ...
            else if (threadState[tile] == 0)
            {
                ...
                else
                {
                    ...
                    if (write)
                    {
                        ...
                        resultsCounter++;
                        WritePoseNMSResult<T>(param, numDetectionsOutput, nmsScoresOutput, nmsClassesOutput,
                            nmsBoxesOutput, nmsKpsOutput, threadScore[tile], threadClass[tile], threadBox[tile],
                            boxIdxMap[tile], keypointsInput, 
                            imageIdx, resultsCounter);
                    }
                }
            }
            else
...
cudaError_t EfficientPoseNMSLauncher(EfficientPoseNMSParameters& param, int* topNumData, int* outputIndexData,
    ...
    const void* boxesInput, const void *keypointsInput, const void* anchorsInput, int* numDetectionsOutput, T* nmsScoresOutput,
    int* nmsClassesOutput, KeyPoints<T>* nmsKpsOutput, void* nmsBoxesOutput, cudaStream_t stream)
{
    ...
    if (param.boxCoding == 0)
    {
        EfficientPoseNMS<T, BoxCorner<T>><<<gridSize, blockSize, 0, stream>>>(param, topNumData, outputIndexData,
            ...
            (BoxCorner<T>*) boxesInput, (KeyPoints<T>*) keypointsInput, (BoxCorner<T>*) anchorsInput, numDetectionsOutput, nmsScoresOutput,
            nmsClassesOutput, nmsKpsOutput, (BoxCorner<T>*) nmsBoxesOutput);
    }
    else if (param.boxCoding == 1)
    {
        ...
        EfficientPoseNMS<T, BoxCenterSize<T>><<<gridSize, blockSize, 0, stream>>>(param, topNumData, outputIndexData,
            outputClassData, sortedIndexData, sortedScoresData, topClassData, topAnchorsData,
            (BoxCenterSize<T>*) boxesInput, (KeyPoints<T>*) keypointsInput, (BoxCenterSize<T>*) anchorsInput, numDetectionsOutput, nmsScoresOutput,
            nmsClassesOutput, nmsKpsOutput, (BoxCorner<T>*) nmsBoxesOutput);
    }
...
size_t EfficientPoseNMSWorkspaceSize(int batchSize, int numScoreElements, int numClasses, int numKeypointsElements, DataType datatype)
{
    size_t total = 0;
...
pluginStatus_t EfficientPoseNMSDispatch(EfficientPoseNMSParameters param, const void* boxesInput, const void* scoresInput,
    const void* keypointsInput, const void* anchorsInput, void* numDetectionsOutput, void* nmsBoxesOutput, void* nmsScoresOutput,
    void* nmsClassesOutput, void* nmsKpsOutput, void* workspace, cudaStream_t stream)
{
    ...
    CSC(cudaMemsetAsync(nmsKpsOutput, 0x00, param.batchSize * param.numOutputBoxes * 17 * 3 * sizeof(T), stream), STATUS_FAILURE);
    ...
    status = EfficientPoseNMSLauncher<T>(param, topNumData, outputIndexData, outputClassData, indexDB.Current(),
        scoresDB.Current(), topClassData, topAnchorsData, boxesInput, keypointsInput, anchorsInput, (int*) numDetectionsOutput,
        (T*) nmsScoresOutput, (int*) nmsClassesOutput, (KeyPoints<T>*) nmsKpsOutput, nmsBoxesOutput, stream);
    CSC(status, STATUS_FAILURE);
    ...
    void* nmsClassesOutput, void* nmsKpsOutput, void* workspace, cudaStream_t stream)
{
    ...
    {
        param.scoreBits = -1;
        return EfficientPoseNMSDispatch<float>(param, boxesInput, scoresInput, keypointsInput, anchorsInput, numDetectionsOutput,
            nmsBoxesOutput, nmsScoresOutput, nmsClassesOutput, nmsKpsOutput, workspace, stream);
    }
    else if (param.datatype == DataType::kHALF)
    {
        ...
        return EfficientPoseNMSDispatch<__half>(param, boxesInput, scoresInput, keypointsInput, anchorsInput, numDetectionsOutput,
            nmsBoxesOutput, nmsScoresOutput, nmsClassesOutput, nmsKpsOutput, workspace, stream);
    }
    else
    {

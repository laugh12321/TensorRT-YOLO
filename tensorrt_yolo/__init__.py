...
from .export import torch_export, paddle_export
from .infer import TRTYOLO, ImageBatcher, TensorInfo, DetectInfo, generate_labels_with_colors, letterbox, scale_boxes, visualize_detections


__all__ = ['trtyolo', 'TRTYOLO', 'ImageBatcher', 'TensorInfo', 'DetectInfo', 'generate_labels_with_colors', 'letterbox', 'scale_boxes', 'visualize_detections', 'torch_export', 'paddle_export']
...
@click.option('--imgsz', default=640, help='Inference image size. Default is 640.', type=int)
...
@click.option('-b', '--batch', default=1, help='Total batch size for the model. Default is 1. Use -1 for dynamic batch size.', type=int)
@click.option('--max_boxes', default=100, help='Maximum number of detections to output per image. Default is 100.', type=int)
@click.option('--iou_thres', default=0.45, help='NMS IoU threshold for post-processing. Default is 0.45.', type=float)
@click.option('--conf_thres', default=0.25, help='Confidence threshold for object detection. Default is 0.25.', type=float)
@click.option('--opset_version', default=11, help='ONNX opset version. Default is 11.', type=int)
@click.option('-s', '--simplify', is_flag=True, help='Whether to simplify the exported ONNX. Default is True.')
def export(model_dir, model_filename, params_filename, weights, version, imgsz, repo_dir, output, batch, max_boxes, iou_thres, conf_thres, opset_version, simplify):
...
def infer(engine, source, output, labels):
    ...
    from .infer import TRTYOLO, ImageBatcher, visualize_detections, generate_labels_with_colors
